...
export const MultiWindowManager: React.FC<MultiWindowManagerProps> = ({
  ...
  return (
    <div className="relative w-full flex-1 min-h-0 h-[calc(100vh-5rem)] flex flex-col bg-slate-950 overflow-hidden select-none">
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-4 py-2 flex items-center justify-between z-30 shrink-0">
        ...
        <div className="flex items-center space-x-1.5">
          <button
            ...
            className="flex items-center space-x-1 px-2.5 py-1 text-xs font-semibold text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg border border-slate-700 transition cursor-pointer"
          ...
          <button
            ...
            className="flex items-center space-x-1 px-2.5 py-1 text-xs font-semibold text-emerald-300 hover:text-white bg-emerald-950/80 hover:bg-emerald-900/80 rounded-lg border border-emerald-700/60 transition cursor-pointer"
        ...
        {windows.map((win) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col bg-slate-900 border rounded-xl shadow-2xl overflow-hidden transition-shadow ${
                isActive 
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 bg-slate-900/95 text-slate-100">
              ...
              {!win.isMaximized && (
                <div
                  ...
                  className="absolute bottom-0 right-0 w-4 h-4 cursor-se-resize flex items-center justify-center text-slate-500 hover:text-emerald-400"
                  title="Drag to Resize"
                >
                  <div className="w-2 h-2 border-r-2 border-b-2 border-slate-500 hover:border-emerald-400" />
                </div>
              )}
            </div>
          );
      ...
      <div className="bg-slate-900 border-t border-slate-800 px-4 py-1.5 flex items-center space-x-2 overflow-x-auto z-40 shrink-0 h-10">
        ...
        {windows.map((win) => {
          ...
          return (
            ...
            >
              ...
              {win.isMinimized && <span className="text-[9px] text-amber-400 font-bold">(min)</span>}
            </button>
          );
        })}

        {windows.length === 0 && (
          <span className="text-xs text-slate-500 italic">No windows open. Click a navigation tab above to open a window.</span>
        )}
      </div>
